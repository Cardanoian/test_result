...
import GradeUserGuide from '@/view/GradeUserGuide';
...
const GradeGenerator: React.FC = () => {
  ...
  const fields: (keyof GradeEvaluationItem)[] = [
    'number',
    'area',
    'standard',
    'element',
  ...
  return (
    <div className='min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 p-4'>
            ...
            <CardContent>
              <div className='rounded-md border'>
                <Table className='w-full table-fixed'>
                  ...
                  <TableBody>
                    {evaluations.map(
                      (item: GradeEvaluationItem, index: number) => (
                        <TableRow key={index}>
      ...
      {isUserGuideOpen && (
        <GradeUserGuide onClose={() => setIsUserGuideOpen(false)} />
      )}
    </div>
  );
};
